...
let quests = {
  packageDelivery: {
    ...
    reward: () => { player.credits += 30; factions.citizens += 2; },
  },
  dataHeist: {
    active: false,
    complete: false,
    desc: "Steal the Data Chip from the club and deliver it to the plaza figure.",
    reward: () => { player.credits += 50; factions.hackers += 3; },
  },
  rareChipTrade: {
    active: false,
    complete: false,
    desc: "Find a hacker interested in the Rare Chip.",
    reward: () => { player.credits += 40; factions.hackers += 2; },
  },
  corpEspionage: {
    active: false,
    complete: false,
    desc: "Infiltrate the luxury store and steal the prototype cyberware.",
    reward: () => { player.inventory.push("Prototype Cyberware"); factions.corp -= 3; factions.mercs += 2; },
  },
};

function updateQuests() {
  const { inventory } = player;
  quests.packageDelivery.active = inventory.includes("Mystery Package");
  quests.packageDelivery.complete = !inventory.includes("Mystery Package") && quests.packageDelivery.active;
  ...
  const scenes = {
    ...
    sneak_gang: {
      text: () => {
        let successChance = player.inventory.includes("Stealth Cloak")
          ? 1
          : randomInt(1, 3);
        if (successChance === 1) {
      ...
      choices: [
        {
          text: "Continue",
          next: () => (player.hp <= 0 ? "game_over" : "club"),
        },
      ],
    },
